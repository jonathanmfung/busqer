...
module Com_canonical_dbusmenu : sig
  val get_layout : OBus_proxy.t -> parentId : int -> recursionDepth : int -> propertyNames : string list -> (int * (int * (string * OBus_value.V.single) list * OBus_value.V.single list)) Lwt.t
  val get_group_properties : OBus_proxy.t -> ids : int list -> propertyNames : string list -> (int * (string * OBus_value.V.single) list) list Lwt.t
  val get_property : OBus_proxy.t -> id : int -> name : string -> OBus_value.V.single Lwt.t
  val event : OBus_proxy.t -> id : int -> eventId : string -> data : OBus_value.V.single -> timestamp : int -> unit Lwt.t
  val event_group : OBus_proxy.t -> events : (int * string * OBus_value.V.single * int) list -> int list Lwt.t
  val about_to_show : OBus_proxy.t -> id : int -> bool Lwt.t
  val about_to_show_group : OBus_proxy.t -> ids : int list -> (int list * int list) Lwt.t
  val items_properties_updated : OBus_proxy.t -> ((int * (string * OBus_value.V.single) list) list * (int * string list) list) OBus_signal.t
  ...
  val icon_theme_path : OBus_proxy.t -> (string list, [ `readable ]) OBus_property.t
end
...
module Org_kde_StatusNotifierItem : sig
  ...
  val icon_accessible_desc : OBus_proxy.t -> (string, [ `readable ]) OBus_property.t
  val attention_icon_name : OBus_proxy.t -> (string, [ `readable ]) OBus_property.t
  val attention_accessible_desc : OBus_proxy.t -> (string, [ `readable ]) OBus_property.t
  ...
  val xayatana_label_guide : OBus_proxy.t -> (string, [ `readable ]) OBus_property.t
  val xayatana_ordering_index : OBus_proxy.t -> (int, [ `readable ]) OBus_property.t
end
