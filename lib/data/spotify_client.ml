...
struct
  ...
  let get_layout proxy ~parentId ~recursionDepth ~propertyNames =
    ...
    let* (revision, layout) = OBus_method.call m_GetLayout proxy (parentId, recursionDepth, propertyNames) in
  ...
  let get_group_properties proxy ~ids ~propertyNames =
    ...
    let* properties = OBus_method.call m_GetGroupProperties proxy (ids, propertyNames) in
    let properties = List.map (fun (x1, x2) -> (Int32.to_int x1, x2)) properties in
    return properties
  ...
  let event_group proxy ~events =
    let events = List.map (fun (x1, x2, x3, x4) -> (Int32.of_int x1, x2, x3, Int32.of_int x4)) events in
  ...
  let about_to_show_group proxy ~ids =
    let ids = List.map Int32.of_int ids in
    let* (updatesNeeded, idErrors) = OBus_method.call m_AboutToShowGroup proxy ids in
  ...
  let items_properties_updated proxy =
    OBus_signal.map
      (fun (updatedProps, removedProps) ->
         let updatedProps = List.map (fun (x1, x2) -> (Int32.to_int x1, x2)) updatedProps in
         let removedProps = List.map (fun (x1, x2) -> (Int32.to_int x1, x2)) removedProps in
  ...
  let text_direction proxy =
    OBus_property.make p_TextDirection proxy

  let status proxy =
    OBus_property.make p_Status proxy

  let icon_theme_path proxy =
    OBus_property.make p_IconThemePath proxy
end
...
struct
  ...
  let new_icon proxy =
    OBus_signal.make s_NewIcon proxy

  let new_icon_theme_path proxy =
    OBus_signal.make s_NewIconThemePath proxy

  let new_attention_icon proxy =
    OBus_signal.make s_NewAttentionIcon proxy

  let new_status proxy =
    OBus_signal.make s_NewStatus proxy

  let xayatana_new_label proxy =
    OBus_signal.make s_XAyatanaNewLabel proxy

  let new_title proxy =
    OBus_signal.make s_NewTitle proxy

  let id proxy =
    OBus_property.make p_Id proxy

  let category proxy =
    OBus_property.make p_Category proxy

  let status proxy =
    OBus_property.make p_Status proxy

  let icon_name proxy =
    OBus_property.make p_IconName proxy

  let icon_accessible_desc proxy =
    OBus_property.make p_IconAccessibleDesc proxy

  let attention_icon_name proxy =
    OBus_property.make p_AttentionIconName proxy

  let attention_accessible_desc proxy =
    OBus_property.make p_AttentionAccessibleDesc proxy

  let title proxy =
    OBus_property.make p_Title proxy
  ...
  let menu proxy =
    OBus_property.map_r_with_context
      (fun context x -> (fun x -> OBus_proxy.make ~peer:(OBus_context.sender context) ~path:x) x)
      (OBus_property.make p_Menu proxy)

  let xayatana_label proxy =
    OBus_property.make p_XAyatanaLabel proxy

  let xayatana_label_guide proxy =
    OBus_property.make p_XAyatanaLabelGuide proxy
